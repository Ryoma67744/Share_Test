...
> **共有相手 (URL を開いた側) も自分の重ね合わせを作れます。** ただし保存先は**その人の端末 (localStorage) だけ**で、共有ドキュメントには送られません。したがって:
> - master が登録したセットは、共有相手からは**読み取り専用** (クリックで切り替えるだけ。✎ / × は出ません)
> - 共有相手が作ったセットは**その人にしか見えず**、master が再 publish しても消えません
> - 再 publish のペイロードにも載らないので、共有相手の見え方が master のプロジェクトへ混ざることはありません
