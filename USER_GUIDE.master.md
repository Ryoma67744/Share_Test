...
### 19-4. 貼り付け / Excel から MRM を一括取り込み

ツールバーの **`一括取り込み`** ボタンを押すと、**貼り付け**と **Excel ファイル**の 2 つのタブが出ます。

#### 貼り付け (Excel でコピーしたセルをそのまま)

Excel で **化合物名 / Precursor / Product / CV / CE** の 5 列を選んでコピーし、`貼り付け` タブの欄に貼り付けるだけです。
...
- 貼り付けると、その場で**解析結果の表**が出ます。**CV と CE がどちらの列に入ったかを必ず目で確かめてください。**
  どちらも小さい整数なので、入れ替わっていても一見それらしく見えてしまいます。
- **1 行目にヘッダ行ごと貼り付けた場合は、列名から順序を自動判別**します。`CE` と `CV` が逆順でも正しく読みます。
- 区切りはタブのほか、カンマ / 2 個以上の空白でも読めます。途中の空行は無視します。
- 同じ化合物名の行が複数あれば、**1 化合物に複数トランジション**としてまとまります。
- **タグ (分類)** と **極性** は表の下でまとめて指定します。極性を「— (名前から推定)」にすると、
  化合物名の `NEG` / `POS` から判定します (`NAD-POS` なら `+`)。
- **登録先** (検証済みライブラリ / 使用実績アーカイブ) と **確認レベル** を選んで **`取り込み実行`**。
...
既存の MRM リスト (xlsx) をそのまま取り込めます。`Excel ファイル` タブの **`ファイルを選択…`** から:
