...
| **重ね合わせの一覧 (Method パネル下部)** | 登録済みのセットが色見本つきで並びます。クリックで表示切替、**`✎` でそのセットを編集** (master のみ) |
...
**セットは何組でも登録できます。** 「Lactate + Citrate」「Glucose + HVA」のように目的別のセットを並べておき、一覧のクリックで見比べてください。`＋重ね合わせ` は押すたびに**新しいセット**を作ります。既に登録したセットを直したいときは、**一覧の各行の `✎`** (プレビュー) か、Method パネルのチップの `✎` (主画面) から開いてください。削除は主画面のチップの `×` です。
