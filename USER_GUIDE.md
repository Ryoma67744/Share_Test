...
> **重ね合わせ (複数分子のカラー合成)**: master が登録した重ね合わせは**画面右端の一覧**に並び、クリックすると表示できます (作成・編集・削除は master のみ)。重ね合わせ表示中は一覧の下の `単一表示へ戻る` で単一化合物の表示に戻せます。合成は**光の足し算**なので、**ある色だけの場所 = その分子だけ / 白い場所 = 重ねた分子の共局在** と読めます。既定は 1 番目マゼンタ・2 番目緑で、この 2 色は光の成分が重ならないため単独と共局在を色だけで区別できます (赤+緑は最も多い色覚型で区別できないため既定にしていません)。加算合成の性質上 **3 分子まで**が実用の目安です。
