...
> **ANALYSIS の数値の出どころ**: バーグラフの Mean Intensity は **元データファイル (xlsx / txt / parquet)
> から直接計算**しています。画面表示用の 8bit 画像は経由しないので、ツールバーの
> **外れ値クリップ**や **Range** をどう変えても数値は変わりません (変わるのは画像の明暗だけ)。
> Export ZIP の Data CSV、MCP コネクタの `get_roi_stats` と同じ値になります。
> **背景除去(Otsu)** を ON にしている場合は、画面から消えている背景画素は平均から除かれます。

> **ANALYSIS の表示切替**: パネル左上のプルダウンで **ROI強度 / 背景除去(Otsu) / KMD図** を切り替えます。
> 初期表示は **背景除去(Otsu)** ですが、**ROI を描き終えた時点で自動的に ROI強度 に切り替わります**。
...
> **Range の初期値と白飛び**: Range に手を触れていないとき、上限は**その分子の強度分布の上位 0.1% の値**に自動設定されます。上限を超えた画素は同じ色に潰れる (白飛びする) ので、**白飛びするのはちょうど 0.10% の画素**です。以前は上位 1% を上限にしていたため 1.00% が白飛びしていました。そのぶん**画像はやや暗くなります** (中央値で 1.4 倍)。**暗く感じたら Range の上限を手で下げてください** — 手入力した値が常に優先され、`Reset` で自動の値に戻ります。
...
| **Range (上部)** | **全切片共通** の vmin / vmax。ここを変更すると全切片の表示レンジが揃います。**Reset** で選択中MRMの手動設定を自動レンジへ戻せます。|
| **背景除去(Otsu) (上部)** | 各ピクセルの総信号量から背景を自動判定して非表示にします。メイン画面ツールバーと**同じ設定**なので、Preview を閉じても残ります (Colormap は閉じると戻ります)。強度の微調整は Preview を閉じて ANALYSIS パネルで行ってください。|
