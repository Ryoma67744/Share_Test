...
> **画像と ROI の向き**: Rotation と MSI 単独回転は主画面・サムネイル・ROI の位置に反映されます。左右／上下反転は操作時の画面の軸を基準にします。共有用に保存された Preview の角度は保持されます。元画像との照合なしに初期方向を一律 180° 変更することはありません。既存 ROI の元座標を復元できない場合は、その状態を確認してから定量に使ってください。
