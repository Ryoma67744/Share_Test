...
- Keep a separate shared-coordinate draft. New sessions default to individual measurements; a valid saved common selection may be restored. Common Save includes every currently registered measurement/reader/revision in `targetKeys`, including unloaded sources. It copies to already-confirmed matching frames for compatibility, and applies to the other registered targets on read after their coordinates are confirmed. Matching coordinate layout alone is not proof of the same physical acquisition; a newly added/replaced source is not silently enrolled in a versioned common setting.
- `MsiAlignmentSession` clones draft data and keeps it out of project metadata until Save. `byFrame` records retain frame and HE/IF identity; `sharedByCoordinate` is an authoritative common record, not only a modal seed. Saved records include their scope and a layer-local monotonic `editOrder`, allocated from the persisted `editCounter` and existing records. The latest explicit edit wins across sessions even when an older exact-frame record belonged to an unloaded source. Legacy records with unknown order retain valid individual precedence. No wall-clock ordering is used.
...
- The main composite, individual Align seeds and aligned HE PNG export resolve the same effective registration. Loading/evicted geometry is pending and is reevaluated on load; no negative resolution is cached and viewing does not write registration records. Legacy common records can repair missing per-frame entries when geometry and HE identity match without a known reader/source mismatch. Malformed, singular, unsupported-basis or replaced-image records cannot fall through to an unaligned HE draw. PNG export pins its MSI target across asynchronous decoding.
