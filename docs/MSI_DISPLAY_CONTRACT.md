...
- Keep one separate common draft per section and HE/IF image identity. Explicit **all sources** applies the same HE-to-MSI logical-pixel transform and points across registered sources even when their acquisition-coordinate origins, intervals, jitter or raster dimensions differ. Do not require exact coordinate equality or introduce a tolerance; do not rescale the saved transform or modify acquisition coordinates. Switching images retains the common draft; its anchor records the last actual edit, not the last visit. New sessions default to individual measurements; a saved common selection may be restored. Common Save includes every currently registered measurement/reader/revision in `targetKeys`, including unloaded sources. It copies to all confirmed frames for compatibility and resolves other registered targets after loading. A newly added/replaced source is enrolled only by a subsequent explicit common edit or Apply-all.
- `MsiAlignmentSession` clones draft data and keeps it out of project metadata until Save. `byFrame` records retain frame and HE/IF identity; the historical `sharedByCoordinate` container holds authoritative common records. Select the latest applicable common record across its coordinate keys. Saved records include their scope and a layer-local monotonic `editOrder`, allocated from the persisted `editCounter` and existing records. The latest explicit edit wins across sessions even when an older exact-frame record belonged to an unloaded source. Legacy records with unknown order retain valid individual precedence. No wall-clock ordering is used. Restoring a draft replaces optional fields as well; an inherited basis or matrix must not turn a navigation-only Save into a new identity registration.
- Explicit Apply-all remains saveable without a parameter change and does not require preloading all images. First selecting a new common scope carries the verified current source alignment (including pending point edits) and records that scope request once; revisiting a common draft or an unchanged Save does not override later individual edits. A default, unverified identity transform is never committed solely by selecting common mode.
- The main composite, individual Align seeds and aligned HE PNG export resolve the same effective registration. Loading/evicted geometry is pending and is reevaluated on load; no negative resolution is cached and viewing does not write registration records. Previously saved common records work without saving again across different acquisition coordinates, provided the current raster is confirmed, HE identity matches and no known reader/source replacement invalidates the target. Malformed, singular, unsupported-basis or replaced-image records cannot fall through to an unaligned HE draw. PNG export pins its MSI target across asynchronous decoding.
...
- Legacy points and unknown frame records remain stored. Individual reuse requires a confirmed source/frame and HE/IF association; explicit common scope reuses the existing logical-pixel values across its targets. Do not guess point conversion or silently erase a record. Unconfirmed displayed frames block point addition and Save with an explanation.
