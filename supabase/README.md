...
- **サーバ一覧の取得**: `[サーバから一覧取得]` を押し、admin password (例: `&lt;CHANGE_ME&gt;`) を入力すると、その admin pw が登録されている全プロジェクトをサーバから列挙
...
`list_projects` は **入力された admin password が一致するプロジェクトだけ** を返します。共通の admin pw をすべてのプロジェクトで使い回せば、1 度の入力で全プロジェクトが取得できます（管理画面「パスワード変更 → サーバから一覧取得のパスワード」で全プロジェクトに一括設定できます）。

> **フロント側に admin パスワードをハードコードしていません**（既定値の pre-fill も撤去済み）。サーバ側の `project_credentials` に bcrypt(cost 12) ハッシュで保存された値と照合するだけです。**旧版で使われていた既知の共通値（例: 過去ドキュメントの `MSIadomine`）は必ず独自の値に変更してください。**
...
適用済みの強化（`share_locks.sql` / `schema.sql` を再実行して反映）:
- パスワードのハッシュコストを **bcrypt cost 12** に引き上げ（既存ハッシュは次回設定時に移行）。
- Storage 書き込みポリシーの slug 前方一致を **`LIKE` から literal 前方一致に変更**（`%`/`_` を含む slug によるパス誤マッチを防止）。`request_publish_session` で slug 文字種を検証。
- フロントの各パスワード prompt から **既定値の pre-fill を撤去**。

既知の制限（設計上の割り切り。運用で留意）:
- **anon による列挙**: 事前プローブと master 側の更新時刻(競合検出)読み取りのため、anon は `projects` の `slug / display_name / is_public / updated_at` を参照できます（private も含む）。中身（`meta` / `anatomy_palette` / ROI 等）は参照できません。
- **未導入（フォローアップ）**: bcrypt 検証RPCのレート制限（総当り対策）、Storage の署名URL化（公開バケットの恒久取得の是正）。
...
2. **マスターパスワードを 1 度だけ設定**:
   ```sql
   select public.set_master_password('&lt;CHANGE_ME&gt;');
   ```
   - 8 文字以上必須
...
4. テスト:
   ```sql
   select public._verify_master_pw('&lt;CHANGE_ME&gt;');  -- → t
   select public._verify_master_pw('wrongpw');     -- → f
   ```

### Master pw のキャッシュ動線

- Master が `index.html` を開いてゲートに `&lt;CHANGE_ME&gt;` を入力
